...
def detect_aruco_codes(frame, aruco_dict, aruco_params):
    ...
    if ids is not None:
        for i, aruco_id in enumerate(ids):
            ...
            cv2.putText(frame, f"ID: {aruco_id}", tuple(pts[0].astype(int)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
...
if not hasattr(cv2, 'aruco'):
    raise ImportError("OpenCV does not have the 'aruco' module. Make sure you have the correct version of OpenCV installed.")
    ...
    if current_arucos:
        for aruco in current_arucos:
            ...
            for i, command in enumerate(commands):
                if command == "centered":
                    cv2.putText(frame, "Centered", (10, 30 * (i + 1)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                else:
                    cv2.putText(frame, command, (10, 30 * (i + 1)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
    else:
        cv2.putText(frame, "No ArUco code detected", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
